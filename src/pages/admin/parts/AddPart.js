...
import { addPart } from '../../../services/admin/PartAdminService';
import PartForm from '../../../components/forms/admin/part/PartForm';

class AddPart extends FormComponent {
    ...
    submit() {
        ...
        addPart(this.state.data).then(response => {

            if(!response.ok) {
                this.props.onFinish(null);
                this.props.enqueueSnackbar(strings.addPart.errorAdding, { variant: 'error' });
                return;
            }

            this.props.enqueueSnackbar(strings.addPart.added, { variant: 'success' });
            this.props.onFinish(null);
        ...
        return (
            <Grid id='page' item md={ 12 }>
                ...
                <div className='header'>
                    <h1>{ strings.addPart.pageTitle }</h1>
                </div>
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(AddPart)));